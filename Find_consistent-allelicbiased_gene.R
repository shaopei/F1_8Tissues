...
MB6_sumlog <- function(Tissue, strand){
  ...
  df$sumlog=result
  df=df[,c(col_to_keep,"sumlog", colnames(df)[grep("V4",colnames(df))],colnames(df)[grep("V9",colnames(df))])]
  write.bed(df[df$sumlog<=0.05,], file=paste(Group, "R1_gencode.vM20.transcript_ABconsistent_FisherMethodP0.05.bed", sep = "_"))
  write.bed(df, file=paste(Group, "R1_gencode.vM20.transcript_ABconsistent.bed", sep = "_"))
}
