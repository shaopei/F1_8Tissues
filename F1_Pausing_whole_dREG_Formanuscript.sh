...
cd /workdir/sc2457/F1_Tissues/Pause_SingleBaseRunOn/whole_dREG_combine_replicate_NEW
...
do
#  bedtools coverage -a <(zcat Browser/${Head}_all.dREG.peak.score.bed.gz| awk 'BEGIN{OFS="\t"} {print $0, ".", "+"} {print $0, ".", "-"}') -b <(zcat map2ref_1bpbed/${Head}_PB6_*_dedup_R1.mat.bowtie.gz_AMBremoved_sorted_specific.map2ref.1bp.sorted.bed.gz) | awk 'BEGIN{OFS="\t"} ($7 >=5){print $1,$2,$3,$4,$5,$6}' > ${intermediate_file}
  bedtools coverage -s -a <(bedtools coverage -s -a ${Head}_${studyBed}_5mat5pat_NS_uniq_labeled.bed -b <(zcat map2ref_1bpbed/${Head}_PB6_*_dedup_R1.mat.bowtie.gz_AMBremoved_sorted_specific.map2ref.1bp.sorted.bed.gz) | awk 'BEGIN{OFS="\t"} ($7 >=5){print $1,$2,$3,$4,$5,$6}') -b <(zcat map2ref_1bpbed/${Head}_PB6_*_dedup_R1.pat.bowtie.gz_AMBremoved_sorted_specific.map2ref.1bp.sorted.bed.gz) | awk 'BEGIN{OFS="\t"} ($7 >=5){print $1,$2,$3,$4,$5, $6}'| sort-bed - |uniq \
...
# dREG sites with KS test fdr<=0.1
...
# dREG sites with KS test fdr >0.9
for Tissue in HT KD SK
do
...
# dREG sites with KS test result labeled and seperate into plus and minus strand
for Tissue in HT KD SK
do
...
# mapped reads from alleleDB pipeline, mapped to diploid genome mat=B6, pat=CAST
for Tissue in HT KD SK
do
  ...
  do
    ...
    do
    	#map3 - mapped postion of the 3 prime end of nascent RNA 
...
do
  ...
  do
    for strand in plus minus
    do
    	#map5 - mapped postion of the 5 prime end of nascent RNA 
...
# mapped reads from proseq2.0 pipeline, mapped to mm10
...
#Pause_SNP_analysis_2020Oct_formanuscript.R
