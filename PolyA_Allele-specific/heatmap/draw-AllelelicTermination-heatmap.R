...
heatmap.AT3 <-function(AT, file.bw.plus.pat,file.bw.minus.pat, file.bw.plus.mat ,file.bw.minus.mat ,dist, step, up_dist =20000, file.pdf="heatmap.pdf", hl.bw.plus.pat=NULL,hl.bw.minus.pat=NULL, hl.bw.plus.mat=NULL ,hl.bw.minus.mat=NULL, bl_wd=1, breaks=NULL, cols=NULL, show.AT.line=TRUE, navg = 10, use.log=FALSE){
AT <- AT[,1:6]
length_order  <- order(AT$V3 - AT$V2, decreasing = T)
AT <- AT[length_order  ,]
# make all beds the same length
...
# get the sum of reads in each bin (size = step)
if (use.log) {
  hmat.pat <- read_read_mat2_log (file.bw.plus.pat, file.bw.minus.pat, bed6[,c(1:6)], step, times=1)
  hmat.mat  <- read_read_mat2_log (file.bw.plus.mat, file.bw.minus.mat, bed6[,c(1:6)],  step, times=1) 
    } else {
  hmat.pat <- read_read_mat2 (file.bw.plus.pat, file.bw.minus.pat, bed6[,c(1:6)], step, times=1)
  hmat.mat  <- read_read_mat2 (file.bw.plus.mat, file.bw.minus.mat, bed6[,c(1:6)],  step, times=1)       
    }
...
## Write out a heatmap.
    if(is.null(breaks)) {
        bk.low <- seq(min(hmat.low), max(hmat.low), 0.1)
    } else {
        bk.low <- breaks
    }
    ...
    if(is.null(breaks)) {
        bk.high <- seq(min(hmat.high), max(hmat.high), 0.1)
    } else {
        bk.high <- breaks
    }
...
between_organs_AT_bw_heatmap_4 <- function (at, t, navg=5){
    # use Alleleic Termination from one organ and see the proseq reads abundance from another organ
AT_intersect <- read.table(paste("../",at,"_AT_3tunitIntersectNativeHMM_intersectRegion.bed", sep=""), header = F)
...
heatmap.AT3(AT_intersect, file.bw.plus.pat,file.bw.minus.pat, file.bw.plus.mat ,file.bw.minus.mat, dist=50000, step=500,  file.pdf=paste(at,"-ATtunitAlleleHMMInterectRegion_", t,"-bw-heatmap-allReads_AT50Kb_step500_navg",navg,".pdf",sep=""), bl_wd=1, show.AT.line=TRUE, navg=navg, use.log=FALSE, breaks=seq(0, 5, 1))
#heatmap.AT3(AT_intersect, file.bw.plus.pat,file.bw.minus.pat, file.bw.plus.mat ,file.bw.minus.mat, dist=100000, step=1000, file.pdf=paste(at,"-ATtunitAlleleHMMInterectRegion_", t,"-bw-heatmap-allReads_AT100Kb_step1K_navg",navg,".pdf",sep=""), bl_wd=1, show.AT.line=TRUE, navg=navg, use.log=TRUE)
#heatmap.AT3(AT_intersect, file.bw.plus.pat,file.bw.minus.pat, file.bw.plus.mat ,file.bw.minus.mat, dist=200000, step=1000,file.pdf=paste(at,"-ATtunitAlleleHMMInterectRegion_", t,"-bw-heatmap-allReads_AT200Kb_step1K_navg",navg,".pdf",sep=""), bl_wd=1, show.AT.line=TRUE, navg=navg, use.log=TRUE)
#heatmap.AT3(AT_intersect, file.bw.plus.pat,file.bw.minus.pat, file.bw.plus.mat ,file.bw.minus.mat, dist=50000, step=500,  file.pdf=paste(at,"-ATtunitAlleleHMMInterectRegion_", t,"-bw-heatmap-allReads_AT50Kb_step500_navg",navg,"_noLine.pdf",sep=""), bl_wd=1, breaks=seq(0,10,1), show.AT.line=FALSE, navg=navg)
#heatmap.AT3(AT_intersect, file.bw.plus.pat,file.bw.minus.pat, file.bw.plus.mat ,file.bw.minus.mat, dist=100000, step=1000, file.pdf=paste(at,"-ATtunitAlleleHMMInterectRegion_", t,"-bw-heatmap-allReads_AT100Kb_step1K_navg",navg,"_noLine.pdf",sep=""), bl_wd=1, breaks=seq(0,10,1), show.AT.line=FALSE, navg=navg)
#heatmap.AT3(AT_intersect, file.bw.plus.pat,file.bw.minus.pat, file.bw.plus.mat ,file.bw.minus.mat, dist=200000, step=1000,file.pdf=paste(at,"-ATtunitAlleleHMMInterectRegion_", t,"-bw-heatmap-allReads_AT200Kb_step1K_navg",navg,"_noLine.pdf",sep=""), bl_wd=1, breaks=seq(0,10,1), show.AT.line=FALSE, navg=navg)
}



between_organs_AT_bw_heatmap_4("BN","BN", navg=10)
between_organs_AT_bw_heatmap_4("LV","LV", navg=10)
between_organs_AT_bw_heatmap_4("LV","BN", navg=10)
between_organs_AT_bw_heatmap_4("BN","LV", navg=10)

between_organs_AT_bw_heatmap_4("BN","BN", navg=5)
between_organs_AT_bw_heatmap_4("LV","LV", navg=5)
between_organs_AT_bw_heatmap_4("LV","BN", navg=5)
between_organs_AT_bw_heatmap_4("BN","LV", navg=5)
