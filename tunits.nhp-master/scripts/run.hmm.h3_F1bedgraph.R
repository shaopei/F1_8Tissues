...
chrom=c("chr1",  "chr10", "chr11", "chr12", "chr13", "chr14", "chr15", "chr16", "chr17","chr18", "chr19", "chr2",  "chr3",  "chr4" , "chr5" , "chr6" , "chr7" , "chr8", "chr9")
#
# prepare data
#
...
save(hmm.params, file=paste(name, "_h3.params.Rdata", sep=''))

# save predictions

# 1. just body
preds.dreg = decode.dataset(hmm.dreg, all.dset.c, 2, covar.lst = covars.clamp)
write.track(preds.dreg, name, name, paste(name, "_h3.preds.bed", sep=''))

# 2. full version
preds.full.dreg = decode.dataset(hmm.dreg, all.dset.c, 2:3, covar.lst = covars.clamp)
write.track(preds.full.dreg, name, name, paste(name, "_h3.preds.full.bed", sep=''))

# 3. extended version
write.extended.track(preds.full.dreg, preds.dreg, name, name, paste(name, "_h3.preds.ext.bed", sep=''))
