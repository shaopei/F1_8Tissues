...
mat<-read.table(paste(Tissue, name_body, "mat.ReadLength.bed", sep = "_"))
pat<-read.table(paste(Tissue, name_body, "pat.ReadLength.bed", sep = "_"))
...
write.table(name, file=paste(Tissue, name_body,"ReadLengthPValue.bed", sep = "_"), quote=F, row.names=F, col.names=F, sep="\t")
