...
domain = read.table("T8_2Strand_p0.05_effect_strain.bed_cluster_organSpecific", header = F)
colnames(domain)=c("chrm", "chrmStart", "chrmEnd", "organ")
rpkm <- merge(rpkm, domain)
...
for (i in 1:dim(rpkm)[1]){
  rpkm$B[i]=rpkm[i,grep(rpkm$organ[i],colnames(rpkm))]
  rpkm$H[i]=max(rpkm[,targetCol][i,grep(rpkm$organ[i],colnames(rpkm)[targetCol], invert = T )])
...
pdf("allellicBias_expression_level2.pdf")
