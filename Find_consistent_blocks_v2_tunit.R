...
MB6_sumlog <- function(Tissue, strand){
  ...
  # use BN_MB6_all_R1_minus_binomtest.bed for df
  df=read.table(paste(Group,"all_R1", strand, "binomtest.bed", sep = "_"))
  ...
  #df=df[,1:3]
  for (pool in c("A", "F", "G")){
    #BN_MB6_G_R1_plus_binomtest.bed
    file = paste(Group, pool, "R1",strand, "binomtest.bed", sep = "_")
    if (file.exists(file)){
  ...
  write.bed(df[df$sumlog<=pvlaue_cutoff,], file=paste(Group,"_tunit_preds_", strand, "_ABconsistent_FisherMethodP",pvlaue_cutoff,".bed", sep = ""))
  write.bed(df, file=paste(Group,"tunit_preds", strand, "ABconsistent.bed", sep = "_"))
}
...
PB6_sumlog <- function(Tissue, strand){
  ...
  # use BN_PB6_all_R1_minus_binomtest.bed for df
  df=read.table(paste(Group,"all_R1", strand, "binomtest.bed", sep = "_"))
  df$winP = substr(df$V4,1,1)
  colnames(df)[grep("V4",colnames(df))]="win_count_all_samples"
  colnames(df)[grep("V9",colnames(df))]="p_value_all_samples"
  ...
  for (pool in c("B", "C", "D", "E")){
    #BN_PB6_B_R1_plus_binomtest.bed
    file = paste(Group, pool, "R1",strand, "binomtest.bed", sep = "_")
    if (file.exists(file)){
      sub_f=read.table(file = file)
      sub_f$winP = substr(sub_f$V4,1,1)
